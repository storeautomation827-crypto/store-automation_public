...
売上・経費・ロス・小口現金などをスマホや PC のブラウザから入力して SQLite に保存し、
確認画面でレビューした後に既存の Excel ファイルへ反映できる仕組みを作っています。

## 作った背景

アルバイト先では、毎日の売上や経費・ロスなどを Excel で管理しています。
入力作業の手間は小さいのですが、同じことを毎日続けるうちに「もう少し入力を楽にできないか」と思い始めました。

ただ、既存の Excel 運用をそのまま維持することが前提だったので、
...
- **日報**: 純売上・組数・客数
- **営業報告**: テキスト入力
- **経費**: 食材（焼き物・加工食品・野菜）・飲料・消耗品など
- **ロス**: 試作・廃棄・調理ミス・注文ミスなど
- **小口現金**: 支払い先・品名・入出金額（品名分類はマッピング設定から選択）
...
**テストコピー先行**
本番 Excel ではなく、事前に準備したコピーに書き込んで結果を確認する手順を踏んでから、
本番に反映するようにしています。
...
CREATE TABLE input_history (
    ...
    input_type    TEXT    NOT NULL,        -- daily_report / petty_cash など
...
実店舗名・実際の売上データ・本番の Excel ファイル・店舗固有の設定ファイルは含みません。
